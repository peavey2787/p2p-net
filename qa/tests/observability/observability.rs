...
fn snapshot_json_includes_accurate_relay_fields() {
    let snap = NodeSnapshot {
        ...
        relay_reservations_accepted_total: 12,
    ...
    assert_eq!(json["relay_reservations_accepted_total"], 12);
...
fn prometheus_metrics_exports_operator_counters() {
    ...
    assert!(metrics.contains("p2p_relay_reservations_accepted_total 12\n"));
...
fn dashboard_distinguishes_discovered_pending_and_connected_peers() {
    ...
    let dashboard =
        fs::read_to_string(root.join("apps/windows/p2p_node.rs")).expect("dashboard example");
    let view = fs::read_to_string(root.join("apps/windows/p2p_node/view/sections.rs"))
...
fn relay_state_updates_snapshot_counters_without_mixing_meanings() {
    ...
    let mut state = RelayState {
        ...
        accepted_reservations_total: 11,
        active_circuits: 4,
    ...
    assert_eq!(snap.relay_reservations_accepted_total, 11);
