...
fn repository_layout_matches_modular_baseline() {
    ...
    assert!(
        windows.contains("P2P_VALIDATION_OUTER_PAUSE")
            && windows.contains("BOOTSTRAP_NO_PAUSE")
            && windows.contains("Validation exited with status"),
        "Windows validation must keep a double-clicked console open even when evidence bootstrap fails before the inner runner starts"
    );
...
fn profile_decisions_are_not_duplicated_in_startup_or_stack_layers() {
    ...
    assert!(
        node_mod.contains("build_swarm(local_key, &cfg, &resolved_config, storage.as_ref())"),
