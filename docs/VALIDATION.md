...
The script remains the canonical one-command runner after the profile/environment refactor. New unit tests such as `environment_detection` are picked up by `cargo test --workspace`, so you do not need a separate command for Phase 2.
