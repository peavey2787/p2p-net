...
DHT provider announce/query begins after a five-second relay/bootstrap head start, retries with 15/30/60-second startup backoff, and can be accelerated after a new external address or recovery from zero connected peers, with a five-second minimum gap. After startup it settles on `discovery.dht.refresh_interval_secs`. This avoids both the startup race where routing is not warm yet and query fan-out starving the public relay connection needed by DCUtR.
