...
`NAT/Public` only shows confirmed public direct or relayed reachability. Local/private listen addresses such as `127.0.0.1`, `172.17.0.1`, RFC1918 LAN ranges, and CGNAT ranges are shown separately as `Local Listen` diagnostics and are not advertised as public addresses. Normal consumer mode also enables `public_ip_probe` by default, which queries HTTPS public-IP endpoints for status only. It does not assume that NAT preserves local listener ports; set `public_ip_probe.advertise_listen_addresses` only when those ports are explicitly forwarded or otherwise known to be externally preserved. AutoNAT status `unknown_no_public_direct_addr_yet` means AutoNAT itself has no public address to test yet; it does not disable the public-IP probe or relay fallback.
...
Consumer mode does not assume one startup DHT query is enough. Relay reservation and bootstrap seeds receive a five-second head start, then DHT announce/query retries back off through 15, 30, and 60 seconds before settling on `discovery.dht.refresh_interval_secs`. Recovery from zero connected peers or learning a new external address can pull the next refresh forward, subject to a five-second minimum gap. Ordinary additional connections do not restart the DHT refresh timer. This lets nodes meet as routing improves without tying DHT work to every application heartbeat or starving the relay connection needed by DCUtR.
