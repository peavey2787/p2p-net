...
| 2026-07-01 | Cleanup | Implemented | Removed stale transitional wording from runtime docs/source comments, replaced hand-maintained snapshot JSON mapping with derived `NodeSnapshot` serialization, added `codebase_hygiene` validation for source/doc hygiene, profile-decision locality, snapshot JSON drift prevention, and unique test registrations, and kept `run-full-validation.ps1` as the one-command validation path. Static edit only; cargo was unavailable in the sandbox for compile/test validation. |
