...
- `discovery.lan`: bounded same-LAN multicast/broadcast discovery. Disabled by default so fresh nodes exercise WAN discovery; enable it explicitly when a local-network fast path is wanted. Discovery hints are compatibility-scoped and the resulting libp2p connection is still authenticated by Noise + Identify.
