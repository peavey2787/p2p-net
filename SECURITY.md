...
- Known-unsound dependencies are release blockers. Unmaintained dependencies are also release blockers except for the exact temporary transitive exception below. No blanket unmaintained allowance is permitted.

### Retired: `RUSTSEC-2025-0141` / `bincode 1.3.3`

The WebRTC 0.17 migration removed `bincode` from the graph (`cargo tree -i bincode` finds no package), so its exception is deleted from both advisory lists. `rkyv` remains in the WebRTC stack and carries a manifest-backed `rkyv >=0.8.18, <0.9` security floor; the hygiene suite enforces both.

### Temporary unmaintained transitive debt

One RustSec unmaintained advisory is temporarily allowlisted because its maintained upstream exit path is not yet available through the supported released dependency families:
...
The advisory ID is explicitly ignored rather than treating all unmaintained crates as warnings. `cargo-audit` denies every other unmaintained advisory, while `cargo-deny` uses `unmaintained = "all"` and fails if the temporary ignore becomes unused. Therefore once an upstream migration removes the crate, validation intentionally fails until its stale exception is deleted; after the exception is removed, all unmaintained dependencies are hard release failures automatically.
