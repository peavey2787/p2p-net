...
fn default_dependency_graph_uses_local_no_hickory_dns_patch() {
    ...
    assert!(
        manifest.contains("libp2p-dns = { path = \"vendor/libp2p-dns\" }"),
...
fn local_dns_patch_does_not_hide_doh_or_hickory() {
    let dns_manifest = include_str!("../vendor/libp2p-dns/Cargo.toml");
    let dns_source = include_str!("../vendor/libp2p-dns/src/lib.rs");
