...
It is based on rust-libp2p `libp2p-webrtc` 0.10.0-alpha and retains the same
public Rust library name (`libp2p_webrtc`) so the transport integration remains
compatible with rust-libp2p 0.57.
...
AES-256-GCM path in the audited `webrtc` 0.17 generation.
